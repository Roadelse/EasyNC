program test_type_direct
    use EasyNC
    implicit none

    character(*), parameter :: fname = 'test.type.nc'
    ...
    t1_s_1%is1 = 2
    t1_s_1%ia1 = [1,3]
    allocate(t1_s_1%iaa1(3:5), source = [1,2,3])

    call easyO(fname, 't1_s_1%is1', t1_s_1%is1)
    call easyO(fname, 't1_s_1%ia1', t1_s_1%ia1)
    call easyI(fname, 't1_s_1%is1', is1)
    call easyI(fname, 't1_s_1%ia1', ia1)
    call assert(is1 .eq. t1_s_1%is1, 'Error in easyIO for type int4 scalar')
    call assert(all(ia1 .eq. t1_s_1%ia1), 'Error in easyIO for type int4 array')

    call easyOA(fname, 't1_s_1%iaa1', t1_s_1%iaa1)
    call easyIA(fname, 't1_s_1%iaa1', iaa1)
    call assert(all(iaa1 .eq. t1_s_1%iaa1), 'Error in easyIO for type int4 allocatable array')
    call assert(all(ubound(iaa1) .eq. 5) .and. all(lbound(iaa1) .eq. 3), 'Error in bounds, in easyIO for type allocatable array')

    print *, 'succeed in test_type'
